...
async function main() {
    ...
    try {
        ...
        // Submit queryAllActiveSLAs transaction
        console.log('Submit queryService transaction.');
        const issueResponse = await contract.submitTransaction('queryAllActiveSLAs');

        // process response
        console.log('Process queryAllActiveSLAs transaction response.');
        let result = JSON.parse(issueResponse.toString());
        console.log(result);
        console.log('Transaction complete. Successfully queried all active SLAs.');
...
main().then(() => {
    console.log('Querying all active SLAs program complete.');
}).catch((e) => {
    console.log('Querying all active SLAs program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
