...
async function main() {
    ...
    try {
        ...
        // delete auctions
        console.log('Submit deleteAllAuctions transaction.');
        const issueResponse = await contract.submitTransaction('deleteAllAuctions');

        // process response
        console.log('Process deleteAllAuctions transaction response.');
        let result = issueResponse.toString();
        console.log(result);
        console.log('Transaction complete. Successfully deleted auctions.');
...
main().then(() => {
    console.log('Delete auctions program complete.');
}).catch((e) => {
    console.log('Delete auctions program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
