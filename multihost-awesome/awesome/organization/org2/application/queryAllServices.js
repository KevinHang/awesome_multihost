...
async function main() {
    ...
    try {
        ...
        // Submit queryAllServices transaction
        console.log('Submit queryService transaction.');
        const issueResponse = await contract.submitTransaction('queryAllServices');

        // process response
        console.log('Process queryAllServices transaction response.');
        let result = JSON.parse(issueResponse.toString());
        console.log(result);
        console.log('Transaction complete. Successfully queried all services.');
...
main().then(() => {
    console.log('Querying all service program complete.');
}).catch((e) => {
    console.log('Querying all services program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
