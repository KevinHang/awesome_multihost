...
async function main() {
    ...
    try {
        ...
        // Submit endSLA transaction
        console.log('Submit endSLA transaction.');
        const issueResponse = await contract.submitTransaction('endSLA', 'sla888');

        // process response
        // console.log('Process endSLA transaction response.');
        // let result = JSON.parse(issueResponse.toString());
        // console.log(result);
        console.log('Transaction complete. Successfully ended the SLA.');
...
main().then(() => {
    console.log('Ending SLA program complete.');
}).catch((e) => {
    console.log('Ending SLA program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
