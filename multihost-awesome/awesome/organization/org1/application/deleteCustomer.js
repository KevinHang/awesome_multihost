...
async function main() {
    ...
    try {
        ...
        // Register a provider
        console.log('Submit deleteCustomer transaction.');
        const issueResponse = await contract.submitTransaction('deleteCustomer');

        // process response
        console.log('Process deleteCustomer transaction response.');
        let result = issueResponse.toString();
        console.log(result);
        console.log('Transaction complete. Successfully deleted customer.');
...
main().then(() => {
    console.log('Delete customer program complete.');
}).catch((e) => {
    console.log('Delete customer  program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
