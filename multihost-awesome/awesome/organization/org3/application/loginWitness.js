...
async function main() {
    ...
    try {
        ...
        // Login a provider
        console.log('Submit loginWitness transaction.');
        const issueResponse = await contract.submitTransaction('loginWitness');

        // // process response
        // console.log('Process loginWitness transaction response.');
        // let result = JSON.parse(issueResponse.toString());
        // console.log(result);
        console.log('Transaction complete. Successfully logged witness in.');
...
main().then(() => {
    console.log('Login witness program complete.');
}).catch((e) => {
    console.log('Login witness  program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
