...
async function main() {
    ...
    try {
        ...
        // Register a provider
        console.log('Submit registerWitness transaction.');
        const issueResponse = await contract.submitTransaction('registerWitness');

        // process response
        console.log('Process registerWitness transaction response.');
        let result = JSON.parse(issueResponse.toString());
        console.log(result);
        console.log('Transaction complete. Successfully registered witness.');
...
main().then(() => {
    console.log('Register wistness program complete.');
}).catch((e) => {
    console.log('Register wtiness  program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
