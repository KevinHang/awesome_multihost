...
async function main() {
    ...
    try {
        ...
        // Register a provider
        console.log('Submit registerCustomer transaction.');
        const issueResponse = await contract.submitTransaction('registerCustomer');

        // process response
        console.log('Process registerCustomer transaction response.');
        let result = JSON.parse(issueResponse.toString());
        console.log(result);
        console.log('Transaction complete. Successfully registered customer.');
...
main().then(() => {
    console.log('Register customer program complete.');
}).catch((e) => {
    console.log('Register customer  program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
