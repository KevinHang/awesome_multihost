...
async function main() {
    ...
    try {
        ...
        // Register a provider
        console.log('Submit deleteWitness transaction.');
        const issueResponse = await contract.submitTransaction('deleteWitness');

        // process response
        console.log('Process deleteWitness transaction response.');
        let result = issueResponse.toString();
        console.log(result);
        console.log('Transaction complete. Successfully deleted witness.');
...
main().then(() => {
    console.log('Delete witness program complete.');
}).catch((e) => {
    console.log('Delete witness program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
