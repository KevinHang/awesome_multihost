...
async function main() {
    ...
    try {
        ...
        // Submit queryService transaction
        console.log('Submit queryService transaction.');
        const issueResponse = await contract.submitTransaction('queryService', 'auction202300013');

        // process response
        console.log('Process queryService transaction response.');
        let result = JSON.parse(issueResponse.toString());
        console.log(result);
        console.log('Transaction complete. Successfully queried service.');
...
main().then(() => {
    console.log('Querying service program complete.');
}).catch((e) => {
    console.log('Querying service program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
