...
async function main() {
    ...
    try {
        ...
        // Submit queryAllSLAs transaction
        console.log('Submit queryService transaction.');
        const issueResponse = await contract.submitTransaction('queryAllSLAs');

        // process response
        console.log('Process queryAllSLAs transaction response.');
        let result = JSON.parse(issueResponse.toString());
        console.log(result);
        console.log('Transaction complete. Successfully queried all SLAs.');
...
main().then(() => {
    console.log('Querying all SLAs program complete.');
}).catch((e) => {
    console.log('Querying all SLAs program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
