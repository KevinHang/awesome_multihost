...
async function main() {
    ...
    try {
        ...
        // Register a provider
        console.log('Submit registerProvider transaction.');
        const issueResponse = await contract.submitTransaction('registerProvider');

        // process response
        console.log('Process registerProvider transaction response.');
        let result = JSON.parse(issueResponse.toString());
        console.log(result);
        console.log('Transaction complete. Successfully registered provider.');
...
main().then(() => {
    console.log('Register provider program complete.');
}).catch((e) => {
    console.log('Register provider  program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
