...
async function main() {
    ...
    try {
        ...
        // Submit endAuction transaction
        console.log('Submit endAuction transaction.');
        const issueResponse = await contract.submitTransaction('endAuction', 'auction202300013', 888);

        // process response
        console.log('Process endAuction transaction response.');
        let result = JSON.parse(issueResponse.toString());
        console.log(result);
        console.log('Transaction complete. Successfully ended the auction.');
...
main().then(() => {
    console.log('Ending auction program complete.');
}).catch((e) => {
    console.log('Ending auction program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
});
